...
fn iterate_file(num: u32, path: &str) -> Result<(), String> {
    let l = read_lines(path).map_err(|err| err.to_string())?;
    for line in l.flatten() {
        ...
        for word in &words[1..] {
            ...
            if start.is_none() {
                ...
                if word.len() == 1 {
                    if start.is_none() {
                        return Err("previous string is not an index for single bit".to_string());
                    }
                ...
                start = match u32::from_str(word) {
                    Err(e) => {
                        return Err(format!("{:?} is a non-range number in the csv {}", word, e));
                    }
                    Ok(v) => Some(v as u8),
                };
            } else {
